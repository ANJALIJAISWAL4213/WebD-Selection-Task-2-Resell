...
const checkout = async (req, res) => {
  ...
  try {
    ...
    // Check if user has the buyer role or seller role to allow checkout for both
    if (user.role !== 'buyer' && user.role !== 'seller') {
      return res.status(403).json({ message: 'Unauthorized: Only buyers and sellers can checkout' });
    }
